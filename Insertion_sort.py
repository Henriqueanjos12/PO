...
for i in range(4):
	#Lista ordenada crescente
	xlc.append(timeit.timeit("insertion({})".format(lc[i]), setup="from __main__ import insertion",number=1))
	ylc.append(tam[i])
	print (xlc[i],",",ylc[i])

	#Lista aleatória com repetição
	xlacr.append(timeit.timeit("insertion({})".format(lacr[i]), setup="from __main__ import insertion",number=1))
	ylacr.append(tam[i])
	print (xlacr[i],",",ylacr[i])
	
	#Lista aleatória sem repetição
	xlasr.append(timeit.timeit("insertion({})".format(lasr[i]), setup="from __main__ import insertion",number=1))
	ylasr.append(tam[i])
	print (xlasr[i],",",ylasr[i])
	
	#Lista ordenada decrescente
	xld.append(timeit.timeit("insertion({})".format(ld[i]), setup="from __main__ import insertion",number=1))
	yld.append(tam[i])
	print (xld[i],",",yld[i])
...
'''
plt.legend(loc=0)
plt.title('Insertion Sort - Luiz Henrique')
plt.xlabel('Tempo(s)')
