...
def bubble(lista):
	...
	for i in range(tamanho_lista-1):
		ord=1
		for j in range(0,tamanho_lista-1,1):
...
for i in range(4):
	#Lista ordenada crescente
	xlc.append(timeit.timeit("bubble({})".format(lc[i]), setup="from __main__ import bubble",number=1))
	ylc.append(tam[i])
	print (xlc[i],",",ylc[i])

	#Lista aleatória com repetição
	xlacr.append(timeit.timeit("bubble({})".format(lacr[i]), setup="from __main__ import bubble",number=1))
	ylacr.append(tam[i])
	print (xlacr[i],",",ylacr[i])
	
	#Lista aleatória sem repetição
	xlasr.append(timeit.timeit("bubble({})".format(lasr[i]), setup="from __main__ import bubble",number=1))
	ylasr.append(tam[i])
	print (xlasr[i],",",ylasr[i])
	
	#Lista ordenada decrescente
	xld.append(timeit.timeit("bubble({})".format(ld[i]), setup="from __main__ import bubble",number=1))
	yld.append(tam[i])
	print (xld[i],",",yld[i])
...
'''
plt.legend(loc=0)
plt.title('Bubble Sort - Luiz Henrique')
plt.xlabel('Tempo(s)')
